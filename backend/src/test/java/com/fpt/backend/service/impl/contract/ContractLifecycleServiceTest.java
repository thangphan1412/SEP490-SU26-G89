...
class ContractLifecycleServiceTest {
    ...
    @Test
    void activatesSignedContractOnEffectiveDateAndRecordsHistory() {
        UUID contractId = UUID.randomUUID();
        LocalDate today = LocalDate.of(2026, 9, 1);
        LocalDateTime activatedAt = LocalDateTime.of(2026, 9, 1, 0, 5);
        Contracts contractReference = new Contracts();

        when(contractRepository.findEffectivePendingContractIds(
                "PENDING_EFFECTIVE",
                today
        )).thenReturn(List.of(contractId));
        when(contractRepository.markEffectiveContractActive(
                contractId,
                "PENDING_EFFECTIVE",
                "ACTIVE",
                today,
        ...
        int count = lifecycleService.activateEffectiveContracts(
                today,
                activatedAt
        );
        ...
        assertThat(historyCaptor.getValue().getToStatus()).isEqualTo("ACTIVE");
        assertThat(historyCaptor.getValue().getAction())
                .isEqualTo("AUTO_ACTIVATE");
    }
